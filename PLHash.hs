...
readBase58 :: Text -> Either Doc Hash
readBase58 txt = do
  case Text.split (== '/') txt of
    (algText:hashText:[])
      -> case readHashAlgorithm algText of
           ...
           _
             -> Left . text $ "Failed to read hashing algorithm"
    _
      -> Left . text $ "Failed to find / separating algorithm and bytes of a Hash"

-- | Attempt to construct a Hash from an algorithm and Base58 encoded Bytes.
mkBase58 :: HashAlgorithm -> Text -> Either Doc Hash
mkBase58 alg txt =
  let ctx err = mconcat
   ...
   in case alg of
        SHA512
          -> case B58.decodeBase58 B58.bitcoinAlphabet . encodeUtf8 $ txt of
               Nothing
                 -> Left . ctx . text $ "Failed to decode base58 bytes from text"

               Just bytes
                 -> let len = BS.length bytes
                     in if len /= 128
                          then Left . ctx . mconcat $
